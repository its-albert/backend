...
if(isset($_GET['action']) && $_GET['action'] == "comments" && isset($_GET['event_id'])){
    ...
    $comments = [];
    $sql = mysqli_query($con, "SELECT c.comment_id, c.user_id, c.event_id, c.user_comment, c.comment_date, u.name
                               FROM comments c
...
if(isset($_POST['action']) && $_POST['action'] == "comment"){
    ...
    if($stmt->execute()){
        echo json_encode(['success'=>1, 'message'=>'Comment added successfully']);
    }else{
